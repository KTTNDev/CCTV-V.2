...
function isStoredUploadTarget(
  value: unknown,
): value is StoredUploadTarget {
  if (!isRecord(value)) {
    return false;
  }
...
function getUploadTargets(
  ...
  if (
    ...
    throw new Error(
      "Request contains invalid upload targets",
    );
  }

  const idCardTargets = uploads.filter(
    (target) => target.kind === "id-card",
  );

  const policeReportTargets = uploads.filter(
    (target) =>
      target.kind === "police-report",
  );

  const sceneTargets = uploads.filter(
    (target) => target.kind === "scene",
  );

  if (
    idCardTargets.length !== 1 ||
    policeReportTargets.length !== 1 ||
    sceneTargets.length > 5
  ) {
    throw new Error(
      "Request contains an invalid upload target set",
    );
  }

  const targetIds = new Set(
    uploads.map((target) => target.id),
  );

  const storagePaths = new Set(
    uploads.map(
      (target) => target.storagePath,
    ),
  );

  if (
    targetIds.size !== uploads.length ||
    storagePaths.size !== uploads.length
  ) {
    throw new Error(
      "Request contains duplicated upload targets",
    );
  }
  ...
  for (const target of uploads) {
    ...
    if (
      !fileName ||
      !target.storagePath.startsWith(
        expectedPrefix,
      )
    ) {
      throw new Error(
        "Request contains an invalid storage path",
      );
    }
  }
...
function assertRequestOwner(
  ...
  if (requestData.ownerUid !== user.uid) {
    throw new HttpError({
      ...
      code: "FORBIDDEN",
      message:
        "คุณไม่มีสิทธิ์ดำเนินการกับคำร้องนี้",
    });
  }
}

function assertDraftIsActive(
  requestData: Record<string, unknown>,
): void {
  if (requestData.status !== "draft") {
    throw new HttpError({
      ...
      code: "CONFLICT",
      message:
        "คำร้องนี้ไม่ได้อยู่ในสถานะรออัปโหลด",
    });
  }

  const draftExpiresAt = getTimestamp(
    requestData.draftExpiresAt,
  );
  ...
  if (
    ...
    throw new HttpError({
      ...
      code: "CONFLICT",
      message:
        "คำร้องหมดเวลาอัปโหลดแล้ว กรุณาเริ่มทำรายการใหม่",
    });
  }
}
  ...
  try {
    ...
    if (
      ...
      throw new HttpError({
        ...
        message:
          `ข้อมูลไฟล์ ${target.originalName} ` +
...
async function verifyAllUploadedFiles(
  targets: StoredUploadTarget[],
): Promise<void> {
  await Promise.all(
    targets.map(verifyUploadedFile),
  );
}
...
function createAttachments(
  ...
  idCard: StoredAttachment;
  policeReport: StoredAttachment;
  scene: StoredAttachment[];
} {
  const idCard = targets.find(
    (target) => target.kind === "id-card",
  );

  const policeReport = targets.find(
    (target) =>
      target.kind === "police-report",
  );
  ...
  if (!idCard || !policeReport) {
    throw new Error(
      "Required upload targets are missing",
    );
  }
  ...
  return {
    ...
    policeReport:
      createStoredAttachment(policeReport),
    scene: targets
      .filter(
        (target) => target.kind === "scene",
      )
...
function createUploadedTargets(
  targets: StoredUploadTarget[],
  uploadedAt: Timestamp,
): Array<
  Omit<StoredUploadTarget, "status"> & {
    status: "uploaded";
    uploadedAt: Timestamp;
  }
> {
...
function createUploadedAuthorizations(
  targets: StoredUploadTarget[],
  uploadedAt: Timestamp,
): Record<
  string,
...
function createExistingResult(
  ...
  if (typeof trackingId !== "string") {
    throw new Error(
      "Request tracking ID is missing",
    );
  }

  const submittedAt =
    getTimestamp(requestData.submittedAt) ??
...
export async function finalizeDraftRequest(
  ...
  const uploadTargets = getUploadTargets(
    input.requestId,
    initialData,
  );

  await verifyAllUploadedFiles(
    uploadTargets,
  );
    ...
    await adminDb.runTransaction(
      async (transaction) => {
        ...
        transaction.update(requestRef, {
          ...
          uploads:
            createUploadedTargets(
              currentTargets,
              submittedAt,
            ),

          uploadAuthorizations:
            createUploadedAuthorizations(
              currentTargets,
              submittedAt,
            ),
          ...
          statusHistory: [
            {
              ...
              note:
                "ได้รับคำร้องและไฟล์แนบครบถ้วนแล้ว",
            },
          ],
