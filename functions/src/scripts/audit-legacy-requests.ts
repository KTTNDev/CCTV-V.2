...
interface AuditCandidate {
  ...
  issues: string[];
  shouldBackfillSchemaVersion: boolean;
}
...
async function main(): Promise<void> {
  ...
  if (!projectId) {
    throw new Error(
      "กรุณาระบุ Firebase project ด้วย " +
      "--project=PROJECT_ID",
    );
  }
  ...
  for (
    ...
    const hasTrackingSecret =
      typeof data.trackingSecretHash ===
        "string" &&
    ...
    if (!phone) {
      issues.push(
        "เบอร์โทรศัพท์ไม่ครบ 4 หลัก",
      );
    }

    if (!eventDate) {
      issues.push(
        "รูปแบบวันที่เกิดเหตุไม่รองรับ",
      );
    }
  ...
  for (const candidate of candidates) {
    if (
      ...
      candidate.issues.push(
        "trackingId ซ้ำมากกว่า 1 เอกสาร",
      );
    }
  }
  ...
  console.log(
    `เอกสารทั้งหมด: ${snapshot.size}`,
  );
  console.log(
    `คำร้องระบบใหม่: ${secureRequestCount}`,
  );
  console.log(
    `คำร้องเก่าที่พร้อมใช้งาน: ${validCandidates.length}`,
  );
  console.log(
    `คำร้องเก่าที่ต้องตรวจสอบ: ${invalidCandidates.length}`,
  );
  console.log(
    `รายการที่จะเติม schemaVersion: ${updateCandidates.length}`,
  );
  ...
  if (
    ...
    for (
      const candidate
      of invalidCandidates.slice(0, 100)
    ) {
      console.log(
        [
